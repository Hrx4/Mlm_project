    ...
    const handleSubmit = async (email , introducerCode , fee)=>{
        try {
            const response = await axios.post(`${backend}/userlist` , {
                userId : userInputs[email] , userEmail : email , introducerCode : introducerCode , membershipFee : parseInt(fee)
            });
            setFirst(!first)
            check.current=true
        ...
        <tbody>
          {
            ...
            <td className=" flex gap-3">
              <button className=" bg-green-500 p-2 rounded-lg  font-bold" disabled={item?.membershipPhoto?false : true} onClick={()=>handleSubmit(item?.userEmail , item?.introducerCode , item?.membershipFee)} >Accept</button>
              <button className=" bg-red-500 p-2 rounded-lg font-bold text-white">decline</button>
              <button className=" bg-blue-500 p-2 rounded-lg font-bold text-white" disabled={item?.membershipPhoto?false : true} onClick={()=>{setOpen(true) , setPhoto(item?.membershipPhoto)}
              }>View</button>
